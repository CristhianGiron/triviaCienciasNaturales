...
function verificarRespuesta(opcionSeleccionada) {
    ...
    if (opcionSeleccionada === preguntaActual.respuesta) {
        ...
        */
        Array.from(opcionesDiv).forEach(boton => {
            if (boton.innerText === opcionSeleccionada) {
                boton.style.boxShadow = "0 0 10px 5px rgba(0, 255, 0, 0.8)"; // Resaltar con neón verde
            }
        });
    } else {
        /*
        ...
        */
        Array.from(opcionesDiv).forEach(boton => {
            if (boton.innerText === opcionSeleccionada) {
                boton.style.boxShadow = "0 0 10px 5px rgba(255, 0, 0, 0.8)"; // Resaltar con neón verde
            }
        });
        // Encontrar el botón correcto y aplicarle un borde verde
        Array.from(opcionesDiv).forEach(boton => {
    ...
    setTimeout(function () {
        const img_container = document.getElementById("img-base");
        img_container.style.boxShadow = "none"; // Elimina la sombra de la imagen
        Array.from(opcionesDiv).forEach(boton => (boton.style.boxShadow = "")); // Eliminar bordes de botones
        siguientePregunta();
